...
def populate_landmarks_dictionary_emotion(landmarks_dictionary: ImageLandmarks, emotion: str, face_boundaries_dict, id: int):
    ...
    emotion_for_id_path = os.path.join("pics_ck+", f"{id}-{emotion}.png")
    if emotion == "neutral":
...
def populate_landmarks_dictionary(landmarks_dictionary: ImageLandmarks, emotions: list):
    id_list = []
    for filename in os.listdir("pics_ck+"):
        id, emotion = filename[:-4].split("-")
        id_list.append(id) if id not in id_list else None

    print(id_list)
    face_boundaries_dict = joblib.load("face_boundaries_dict_ck+.pkl")
    for id in id_list:
...
if __name__ == '__main__':
    ...
    # joblib.dump(landmarks_dictionary, "landmarks_ck+.joblib")
    landmarks_dictionary = joblib.load("landmarks_ck+.joblib")
    ...
    for index, probability in enumerate(probabilities):
        ...
        if true_label == "anger" and np.argmax(probability) != 0: # 0 is anger's index
            print(probability)
    ...
    # single_prediction = clf.predict_proba(X_test[1].reshape(1, -1))
